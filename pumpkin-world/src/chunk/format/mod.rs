...
impl ChunkData {
    ...
    pub fn internal_from_bytes(
        ...
        // EMBER start - chunk-embedded storage for ember custom blocks/furniture
        let mut ember_custom_blocks = FxHashMap::default();
        if let Some(list) = root_tag.get_list("ember_custom_blocks") {
            for tag in list {
                if let pumpkin_nbt::tag::NbtTag::Compound(entry) = tag
                    && let (Some(x), Some(y), Some(z), Some(block_id)) = (
                        entry.get_int("x"),
                        entry.get_int("y"),
                        entry.get_int("z"),
                        entry.get_string("block_id"),
                    )
                {
                    ember_custom_blocks.insert(BlockPos::new(x, y, z), block_id.to_owned());
                }
            }
        }

        let mut ember_furniture = Vec::new();
        if let Some(list) = root_tag.get_list("ember_furniture") {
            for tag in list {
                if let pumpkin_nbt::tag::NbtTag::Compound(entry) = tag
                    && let (
                        Some(instance_id),
                ...
                {
                    ember_furniture.push(EmberFurnitureEntry {
                        instance_id,
        ...
        Ok(Self {
            ...
            // EMBER start - chunk-embedded storage for ember custom blocks/furniture
            ember_custom_blocks: std::sync::Mutex::new(ember_custom_blocks),
            ember_furniture: std::sync::Mutex::new(ember_furniture),
            // EMBER end
    ...
    fn internal_to_bytes(&self) -> Result<Bytes, ChunkSerializingError> {
        ...
        // EMBER start - chunk-embedded storage for ember custom blocks/furniture
        let custom_block_list = ember_custom_blocks_nbt
            .into_iter()
            .map(|entry| {
                let mut compound = NbtCompound::new();
                compound.put_int("x", entry.x);
                compound.put_int("y", entry.y);
                compound.put_int("z", entry.z);
                compound.put_string("block_id", entry.block_id);
                NbtTag::Compound(compound)
            })
            .collect();
        root_compound.put_list("ember_custom_blocks", custom_block_list);

        let furniture_list = ember_furniture_nbt
            .into_iter()
            .map(|entry| {
                let mut compound = NbtCompound::new();
                compound.put_string("instance_id", entry.instance_id.simple().to_string());
                compound.put_string("furniture_id", entry.furniture_id);
                compound.put_double("x", entry.x);
                compound.put_double("y", entry.y);
                compound.put_double("z", entry.z);
                compound.put_float("yaw", entry.yaw);
                NbtTag::Compound(compound)
            })
            .collect();
        root_compound.put_list("ember_furniture", furniture_list);
        // EMBER end
