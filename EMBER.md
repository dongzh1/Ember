...
| 资源包生成器（自托管/S3，参考 CraftEngine 阶段一） | `pumpkin-config/src/resourcepack_builder.rs`（`ResourcePackBuilderConfig`/`HostingMode`/`SelfHostedConfig`/`S3Config`）、`pumpkin/src/server/resourcepack_builder.rs`、`pumpkin/src/server/mod.rs`（`Server::new` 里的接线） | 移植 `craft-engine`(Paper 自定义物品/方块/家具插件)部分功能的四阶段计划(地基→自定义物品→自定义家具→自定义方块,详见 plan 文档)里的第一阶段。**先更正一处自己的误判**:一开始以为 Java 版资源包推送完全没接线(grep 包名 `CAddResourcePack` 零命中)——深查后发现记错了类型名,真实类型是 `CConfigAddResourcePack`,其实已经在 `net/java/login.rs`(`handle_login_acknowledged`)完整接好,客户端回复也在 `net/java/config.rs`(`handle_resource_pack_response`)里真的处理。**这套发送/应答逻辑完全没有改动**,这次新增的是"包从哪来"这一层:`Server::new` 里新起的这段逻辑在**已有发送逻辑读取配置之前**,把 `advanced_config.resource_pack.java` 的 `url`/`sha1`/`enabled` 三个字段程序化覆盖掉,下游代码对"包是自己生成的还是管理员手动配的外部 URL"完全无感——两种用法可以共存(不开生成器,手动配 `[resource_pack]` 外部 URL 的老用法不受影响)。生成流程:读 `resourcepack/source/`(`pack.mcmeta`+`assets/...`,首次不存在会落一个最小占位 `pack.mcmeta`)→ 用新增的 `zip` crate 打包进内存(不落地临时文件)→ 用已有的 `sha1` crate 算 hash。**两种托管方式**(`hosting = "self_hosted" | "s3"`):自托管起一个 `tiny_http` 服务器(新依赖,专门配一个独立 OS 线程跑,不占 tokio 运行时——这种低频"整个 GET 请求就是吐字节"场景没必要占用异步运行时);S3(老大中途明确要求要支持,不只是自托管)把包 `PUT` 到任意 S3 兼容存储(真实 AWS/Cloudflare R2/MinIO 等,自定义 `endpoint` 时走 path-style 寻址不依赖每桶 DNS),签名用新增的 `aws-sigv4`+`aws-credential-types`(先派 WebFetch 核实了这两个包的准确 API 签名再落笔,包括 `sign-http` 特性单独还不够、必须同时打开默认的 `http1` 特性这个坑,以及用 `SigningInstructions::headers()` 拿裸 `(&str,&str)` 键值对而不是绑定 `http` crate 的 `Request` 类型——这样才能套到已有的 `ureq` 请求构建器上)而不是手搓签名算法本身(SigV4 是安全相关协议，手搓比用现成库风险高)。**验证边界(诚实披露)**:自托管模式端到端实测过(起服→内存打包→真实 `curl` 下载→验证是合法 zip 且内容正确);S3 模式编译通过、clippy 干净、严格照文档写的调用方式，但**没有真实 S3/R2/MinIO 凭据可供实测**，属于"文档正确性有把握、真实环境未验证"。**范围外**:资源包本身内容(自定义物品模型/贴图等)是阶段二的事,这一步只关心"包能不能正确生成+送到客户端手上" |
| 自定义物品（`item_model` 组件，CraftEngine 阶段二） | `pumpkin-config/src/custom_item.rs`（`CustomItemListConfig`/`CustomItemConfig`）、`pumpkin/src/server/custom_item.rs`（`CustomItemManager`，含单元测试）、`pumpkin/src/command/commands/custom_item.rs`（`/customitem give\|list`） | CraftEngine 移植四阶段计划的第二阶段。协议里没有"新物品 ID"这个概念，自定义物品的地基原来就是现成的：`DataComponent::ItemModel`（1.21.2+ 新协议）已经完整实现 NBT+wire codec（每个原版物品本来就带一份默认值），`ItemStack.patch: Vec<(DataComponent, Option<Box<dyn DataComponentImpl>>)>` 已经支持运行时覆盖任意组件——`add_enchantment` 是现成先例（`patch.push((DataComponent::Enchantments, Some(Box::new(...))))`），照抄这个写法给 `ItemStack` 覆盖 `ItemModel` 指向自定义模型路径即可，`display_name` 同理覆盖 `DataComponent::CustomName`（也已完整实现）。**范围决定**：`DataComponent::CustomModelData`（老协议数值型自定义模型，1.21.2 之前客户端要用这个）是个空壳——NBT/wire codec 三处 dispatch 都不认识这个类型，发送会直接报错"not yet implemented"，从零实现需要写字段（浮点数组/flags/字符串/颜色，对齐 1.20.5+ 规范）+ NBT 编解码 + wire codec + 三处 dispatch 注册，是一块独立不小的活。v1 先只做新协议，`item_model` 已覆盖 Ember 重心版本（v26.2）；老客户端连上会看到自定义物品显示成原版物品本体的样子（不会崩，只是没有自定义外观）。**配置**（`resourcepack/items.toml`，`LoadConfiguration`，不用 `#[serde(transparent)]`——踩过这个坑）：每条 `id`/`base_item`(真实原版物品资源名，决定最大堆叠数/耐久等一切原版行为)/`model`(资源包里的模型路径，需要和阶段一资源包生成器的 `source_dir/assets/...` 里有对应文件，两个系统之间没有代码耦合，只是约定同一份 `assets` 目录)/可选 `display_name`。**指令**：`/customitem give <玩家> <id> [数量]`（op 3 级）——数量超过物品最大堆叠数会自动按 `/give` 同款逻辑拆成多组塞背包；`/customitem list` 列出所有配置的 id。这次只做"能拿到手"的验证入口，不做自动接入商店/合成表（后续按需再加）。**验证**：新增 4 个纯逻辑单元测试（`stack_from_entry` 不需要网络/数据库/客户端，纯函数式验证 patch 构造是否正确）全部通过 + 起服验证配置自愈正常，但**没有真实 Java 客户端连接验证贴图渲染效果**——需要真的准备模型/贴图素材放进阶段一的资源包才能看到实际外观，这部分素材制作不是代码工作 |
| 自定义家具（展示实体+边表，CraftEngine 阶段三；顺带新增 `LoadConfiguration::save`） | `pumpkin-config/src/furniture.rs`（`FurnitureListConfig`/`FurnitureInstanceListConfig`）、`pumpkin/src/server/furniture.rs`（`FurnitureManager`）、`pumpkin/src/net/java/play.rs`（`handle_use_item_on` 放置钩子、`handle_attack`/`handle_interact` 破坏钩子）、`pumpkin-config/src/lib.rs`（`LoadConfiguration::save` 新方法） | CraftEngine 移植四阶段计划的第三阶段。架构上和发包 NPC 系统（`server::npc::NpcManager`）是同一路数：一个家具实例 = 持久化的 `FurnitureInstanceConfig`（世界/坐标）+ 运行时 `RuntimeFurniture`（假 entity_id + 当前可见玩家集合），可见性判定照抄 `NpcManager::tick` 同一套 `chunker::is_within_view_distance` 规则、每 10 tick 复查一次。和 NPC 的结构性差异只有渲染方式：家具是 `item_display`（渲染成阶段二某个自定义物品同款模型，**踩过一次坑**——最初实现只发了裸 `ItemStack::new(1, item)` 没带 `ItemModel` 补丁，clippy 的"字段从未读取"警告顺手揪出了这个本会导致家具渲染成原版物品本体的真 bug，修成和 `CustomItemManager::stack_from_entry` 一样补丁写法）+ 一个 `interaction` 点击判定箱（和悬浮菜单系统同一套发包技术）。**放置**：`handle_use_item_on`（`net/java/play.rs`）开头新增一段检查——右键点方块时，先看手持物品的 `ItemModel` 组件是不是某个配置了的自定义物品、那个自定义物品是不是配置成了家具，命中就直接放置家具、扣一个手持物品、`return`，完全跳过后面的原版方块交互/放置逻辑，不去碰那段本来就很复杂的既有代码。**破坏**：`handle_attack`/`handle_interact` 两处（和 NPC/菜单点击同样的"两个包处理路径都要拦"教训）新增家具判定分支，命中就摘除记录+从当前所有观察者客户端移除+把对应自定义物品还给玩家（走 `offer_or_drop_stack`，背包有空位就进背包，满了才丢在地上）。**持久化新增的基础设施**：这是仓库里第一个"运行时会反复增删"的 TOML 配置（`furniture/instances.toml`）——之前所有 TOML 配置（商店/菜单等）都是纯管理员静态配置，只在启动时加载，从没需要过"运行时改完再写回磁盘"这个能力；`LoadConfiguration` trait 原来确实没有这个方法（`load()` 内部的自愈写回是一次性的，不是通用能力），这次给 trait 补了一个默认实现的 `save(&self, config_dir)` 方法，任何 TOML 配置以后要做类似"运行时可变+持久化"的功能都能直接复用，不用each自己再手搓一遍。**验证**：编译+clippy(debug/release)全过+起服验证两个配置文件自愈正常；**没有真实客户端验证放置/破坏的实际手感**（这两个操作依赖真实的方块交互/攻击包，没法在没有客户端连接的环境里模拟），已尽量让代码路径经过仔细复核（这次复核过程本身就抓出了上面那个真实的 ItemModel 遗漏 bug） |
| 自定义方块（原版方块载体+展示实体两种，CraftEngine 阶段四/收尾） | `pumpkin-config/src/custom_block.rs`（`CustomBlockListConfig`/`CustomBlockInstanceListConfig`）、`pumpkin/src/server/custom_block.rs`（`CustomBlockManager`）、`pumpkin-config/src/furniture.rs`（`RenderMode::Block` 分支）、`pumpkin/src/server/furniture.rs`（`FurnitureManager` 按 `render_mode` 分渲染 `item_display`/`block_display`）、`pumpkin/src/block/registry.rs`（`on_use` 拦截点）、`pumpkin/src/world/mod.rs`（`break_block` 掉落覆盖点）、`pumpkin/src/net/java/play.rs`（`handle_use_item_on` 放置分支） | CraftEngine 移植四阶段计划的收尾阶段——老大在阶段三验收后明确要求"完整的做，展示实体方块和原版方块两种都要"。**两种机制**：①**展示实体方块**——不新起一个 manager，直接给阶段三的 `FurnitureConfig` 加一个 `render_mode: Item(默认)\|Block` 分支，完全复用家具现成的持久化/视距广播/放置/破坏逻辑，只有渲染方式不同：`Block` 模式发 `block_display` 实体 + `TrackedData::BLOCK_STATE`(`Metadata::write` 对 `MetaDataType::BLOCK_STATE` 自动做跨协议版本的状态 id 重映射，直接传目标方块的 `state_id` 即可，不用自己处理版本差异)、且不设 `BILLBOARD`(方块该有固定朝向，不像图标那样一直转向摄像机)。②**原版方块状态载体**(风险最高的部分，动工前专门用 `AskUserQuestion` 征询过老大——这一步要改的是 `block/registry.rs`/`world/mod.rs` 这些**所有方块共享**的核心分发代码，不再像前三阶段那样只加独立新文件)：`CustomBlockManager` 自己维护一个独立的位置索引(`blocks/instances.toml`，`(世界名, 坐标) -> 自定义方块 id`)，**没有**使用 `BlockEntity`——调研过 `block_entity_from_nbt`(`block/entities/mod.rs`)是一个封闭的 `match` 语句，接入自定义 block entity 类型要再加一处核心文件改动才能在重启后正确从磁盘 NBT 重建，用一份独立索引能达到同样效果又不用碰它；载体自己真实的方块状态照常走原版世界存档格式(它本来就是个真方块)，这份索引只是额外记"这个位置其实是哪个自定义方块"。拦截点：`BlockRegistry::on_use`(查到记录直接 `BlockActionResult::Success`，不跑载体自己的交互逻辑，比如音符盒调音)、`World::break_block`(查到记录时不掉落载体的原版战利品表，改成掉落对应的自定义物品并从索引摘除记录)——两处都是"先查有没有记录，没有就完全落回原来那一行代码"的插入方式，不改动任何单个方块自己的文件(`note.rs`一行不动)。**范围收窄**：v1 只支持 `note_block` 一种载体(蘑菇块/绊线钩/树叶暂未做，机制打通后可以照同一模式加)，选它是因为碰撞箱/透明度合适、且它的"根据上下相邻方块自动重算 `instrument` 属性"这一原版行为不影响自定义方块*识别*(识别只查索引，从不看载体自己的状态)；右键交互 v1 直接吞掉，不做自定义方块自己的交互行为。**安全验证边界(诚实披露)**：两处核心分发的插入点都逐字对照过——没有记录的位置保证完全落回原来那行代码，字节不差；但没有真实客户端能实测验证普通音符盒等原版方块行为确实没被改坏，只能靠代码审查本身担保，这一点在动工前就已经如实告知老大并取得明确同意后才继续。至此 CraftEngine 移植四阶段计划全部完成 |
