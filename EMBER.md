...
| 动态世界管理 | `pumpkin/src/command/commands/world.rs`、`pumpkin/src/command/commands/mod.rs`、`pumpkin/src/server/mod.rs` | `/world list/load/unload/tp/clone/prewarm/convert`（权限 `ember:command.world`，OP 3 级）；`Server::unload_world` 运行时卸载(撤离玩家→移出 tick→后台存盘停机)；`create_world_with(Option<LevelConfig>)` 按世界配置建世界 |
