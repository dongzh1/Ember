...
| 副本零留存卫生 | `pumpkin-world/src/level.rs`（`Level::ephemeral`）、`pumpkin/src/world/mod.rs`、`pumpkin/src/command/commands/dungeon.rs` | 只读克隆世界不建 region/entities/poi 文件夹、不落 POI、`/dungeon stop` 卸载后清理残留目录。`/dungeon` = 给模板世界开自动命名的只读克隆 |
...
| 动态世界管理 | `pumpkin/src/command/commands/world.rs`、`pumpkin/src/command/commands/dungeon.rs`、`pumpkin/src/command/commands/mod.rs`、`pumpkin/src/server/mod.rs` | `/world list/load/unload/tp/clone/prewarm/convert` + `/dungeon prewarm/start/stop/list/reload`（权限 `ember:command.world`/`ember:command.dungeon`，OP 3 级）；`Server::unload_world` 运行时卸载(撤离玩家→移出 tick→后台存盘停机)；`create_world_with(Option<LevelConfig>)` 按世界配置建世界 |
