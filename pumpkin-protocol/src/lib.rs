    ...
    pub fn write(&self, write: &mut impl ser::NetworkWriteExt) -> Result<(), ser::WritingError> {
        match &self.kind {
            ResolvableProfileKind::Partial {
                ...
                write.write_option(username, |w, value| w.write_string(value))?;
                write.write_option(uuid, ser::NetworkWriteExt::write_uuid)?;
                write.write_list(properties, |writer, property| property.write(writer))?;
            }
            ResolvableProfileKind::Complete(profile) => {
                write.write_var_int(&VarInt(1))?;
                write.write_uuid(&profile.uuid)?;
                write.write_string(&profile.name)?;
                write.write_list(&profile.properties, |writer, property| {
